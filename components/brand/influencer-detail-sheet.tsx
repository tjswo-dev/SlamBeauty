...
import {
  X, Star, MapPin, Users, TrendingUp, Clock, CheckCircle,
  MessageSquare, ExternalLink, ThumbsUp, BarChart2, Shield,
} from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";

export type SheetInfluencer = {
  id: string;
  name: string;
  handle: string;
  avatar: string;
  ...
  portfolio?: { brand: string; result: string; link: string }[];
...
interface InfluencerDetailSheetProps {
  influencer: SheetInfluencer | null;
  onClose: () => void;
...
export function InfluencerDetailSheet({
  ...
  const hasAnalytics =
    ...
    influencer.rating !== undefined ||
    influencer.completedCampaigns !== undefined ||
    influencer.responseRate !== undefined;

  const hasAudience =
    influencer.audienceGender !== undefined ||
    influencer.audienceAge !== undefined ||
    influencer.countryLabel !== undefined;

  const hasRateCard =
    influencer.exclusiveRate !== undefined && influencer.regularRate !== undefined;

  const discount =
    hasRateCard
      ? Math.round((1 - influencer.exclusiveRate! / influencer.regularRate!) * 100)
      : 0;
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-[480px] bg-white shadow-2xl z-50 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {hasAnalytics && (
            <div className="px-6 py-5 border-b border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "팔로워", value: formatNumber(influencer.followers), icon: Users, color: "text-indigo-600", bg: "bg-indigo-50", show: true },
                  { label: "참여율", value: influencer.engagementRate !== undefined ? `${influencer.engagementRate}%` : null, icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50", note: "업계 평균 2.8%", show: influencer.engagementRate !== undefined },
                  { label: "평균 좋아요", value: influencer.avgLikes !== undefined ? formatNumber(influencer.avgLikes) : null, icon: ThumbsUp, color: "text-purple-600", bg: "bg-purple-50", show: influencer.avgLikes !== undefined },
                  { label: "평균 댓글", value: influencer.avgComments !== undefined ? formatNumber(influencer.avgComments) : null, icon: MessageSquare, color: "text-amber-600", bg: "bg-amber-50", show: influencer.avgComments !== undefined },
                  { label: "완료 캠페인", value: influencer.completedCampaigns !== undefined ? `${influencer.completedCampaigns}건` : null, icon: CheckCircle, color: "text-slate-600", bg: "bg-slate-100", show: influencer.completedCampaigns !== undefined },
                  { label: "응답률", value: influencer.responseRate !== undefined ? `${influencer.responseRate}%` : null, icon: Clock, color: "text-purple-600", bg: "bg-purple-50", show: influencer.responseRate !== undefined },
                ].filter((m) => m.show && m.value).map((m) => (
                  <div key={m.label} className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 border border-slate-100">
                    ...
                    <div>
                      ...
                      {"note" in m && m.note && <div className="text-[10px] text-slate-400">{m.note}</div>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="px-6 py-5 border-b border-slate-100">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">
                <span className="flex items-center gap-1.5">
                  <BarChart2 className="w-3.5 h-3.5" />오디언스 분석
                </span>
              </h3>
                ...
                {influencer.audienceGender && (
                  <div>
                    ...
                    <div className="flex rounded-full overflow-hidden h-2">
                      <div className="bg-purple-400 h-full" style={{ width: `${influencer.audienceGender.female}%` }} />
                      <div className="bg-indigo-300 h-full flex-1" />
                    </div>
                  </div>
                )}
                {influencer.audienceAge && Object.keys(influencer.audienceAge).length > 0 && (
                  <div>
          ...
          {(influencer.categories.length > 0 || influencer.platforms.length > 0) && (
            <div className="px-6 py-5 border-b border-slate-100">
              ...
              <div className="space-y-2">
                {influencer.portfolio.map((p, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <a href={p.link} className="p-1.5 rounded-lg hover:bg-white transition-colors text-slate-400 hover:text-indigo-500">
              ...
              <div className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-4">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-indigo-200">
                  <div className="flex items-center gap-1.5 text-xs text-emerald-700">
                    <Shield className="w-3.5 h-3.5" />SlamBeauty 전용 할인
                  </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50">
          {mode === "approve" ? (
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1 border-red-200 text-red-600 hover:bg-red-50" onClick={onReject}>거절</Button>
              <Button className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white" onClick={onApprove}>
            ...
            <div className="flex items-center justify-center gap-2 py-2 text-emerald-600 font-semibold">
              <CheckCircle className="w-4 h-4" />협업 요청이 전송되었습니다
            </div>
          ) : (
            <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white" onClick={handleRequest}>
              협업 요청 보내기
            </Button>
          )}
        </div>
